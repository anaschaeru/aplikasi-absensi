                ...
                <div class="flex flex-col md:flex-row justify-between items-start mb-4 gap-4">
                  <div>
                    ...
                    <div class="mt-1 text-sm text-gray-500 space-y-1">
                      <p>Kelas: <span class="font-medium text-gray-700">{{ $rekapData['info']['nama_kelas'] }}</span></p>
                      {{-- Tampilkan nama mapel jika dipilih --}}
                      @if ($selectedMapelId && isset($mapelList[$selectedMapelId]))
                        <p>Mapel: <span class="font-medium text-gray-700">{{ $mapelList[$selectedMapelId] }}</span></p>
                      @endif
                      <p>Periode: <span class="font-medium text-gray-700">{{ $rekapData['info']['periode'] }}</span></p>
                    </div>
                  </div>
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($rekapData['rekap'] as $data)
                      <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $data['nama_siswa'] }}</td>
                        <td class="px-6 py-4 text-sm text-center font-bold text-green-700 bg-green-50/50">
                          {{ $data['kehadiran']['Hadir'] }}</td>
                        <td class="px-6 py-4 text-sm text-center font-bold text-yellow-700 bg-yellow-50/50">
                          {{ $data['kehadiran']['Sakit'] }}</td>
                        <td class="px-6 py-4 text-sm text-center font-bold text-blue-700 bg-blue-50/50">
                          {{ $data['kehadiran']['Izin'] }}</td>
                        <td class="px-6 py-4 text-sm text-center font-bold text-red-700 bg-red-50/50">
