...
@section('content')
  <div x-data="attendanceApp()" class="bg-gray-100 min-h-screen">
    <form action="{{ route('guru.absensi.store', ['jadwal' => $jadwal->jadwal_id]) }}" method="POST">
      @csrf
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
          {{-- HEADER TABEL (th) --}}
          <div class="grid grid-cols-[auto_1fr_auto_auto] gap-x-4 items-center p-4 bg-gray-50 border-b border-gray-200">
            ...
            <span class="text-xs font-semibold uppercase tracking-wider text-gray-500 text-center">Catatan</span>
          </div>

          <div class="divide-y divide-gray-200">
            @foreach ($jadwal->kelas->siswa as $siswa)
              {{-- Default status adalah 'Hadir' dan catatan tersembunyi --}}
              <div x-data="{ selectedStatus: 'Hadir', showNotes: false }">
                {{-- STRUKTUR BARIS SISWA MENGGUNAKAN GRID --}}
                <div class="grid grid-cols-[auto_1fr_auto_auto] gap-x-4 items-center p-4">
                  ...
                  {{-- Kolom 2: Info Siswa --}}
                  <div>
                    <div class="text-xs text-gray-500">{{ $siswa->nis ?? 'N/A' }}</div>
                    <div class="font-semibold text-gray-800">{{ $siswa->nama_siswa ?? 'N/A' }}</div>
                  </div>

                  {{-- Kolom 3: Status Kehadiran --}}
                  <div class="flex rounded-lg border border-gray-300 p-0.5">
                    @foreach (['Hadir', 'Sakit', 'Izin', 'Alfa'] as $status)
                      <label @click="selectedStatus = '{{ $status }}'" class="relative cursor-pointer">
                        <input type="radio" name="absensi[{{ $siswa->siswa_id }}][status]" value="{{ $status }}"
                          class="sr-only" x-model="selectedStatus" required>
                  ...
                  {{-- Kolom 4: Tombol Catatan --}}
                  <button @click="showNotes = !showNotes" type="button"
                    class="p-2 mx-auto rounded-full text-gray-400 hover:bg-gray-100 hover:text-gray-600"
                ...
                <div x-show="showNotes" x-transition class="px-4 pb-4">
                  <input type="text" name="absensi[{{ $siswa->siswa_id }}][catatan]"
      ...
      <div class="sticky bottom-0 z-10 bg-white/80 backdrop-blur-sm border-t border-gray-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex justify-between items-center py-4">
            <a href="{{ route('guru.dashboard') }}" class="text-sm font-medium text-gray-600 hover:text-gray-800">
              Batal
            </a>
            <button type="submit"
